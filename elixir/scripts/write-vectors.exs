...
packages =
  # Each path starts at the root of the repository, so that both
  # implementations read the same file.
  for path <- [
        "cmd/exec-provider/manifest.json",
        "elixir/test/fixtures/providers/sqlite-provider.json",
        "elixir/test/fixtures/providers/users-provider.json"
      ] do
    {:ok, package} = CapabilityPackage.load_file(Path.expand("../" <> path))
...
path = Path.expand("../test/vectors/identity.json")
