...
installed ARC v0.3.2 on the host to connect through the pinned relay. Only the
relay publishes a host port, bound to `127.0.0.1`. Agent keys stay on the host.

The service build packages the providers with the released ARC `0.3.2` runtime.
...
From the repository root, if you have not installed v0.3.2:

```sh
ARC_VERSION=0.3.2 sh install.sh
...
```sh
docker build -t arc-local-preview:0.3.2 .
ARC_IMAGE=arc-local-preview:0.3.2 docker compose up -d --build --wait
```
...
agents before sending a DM. In v0.3.2, `publish` populates the local public
