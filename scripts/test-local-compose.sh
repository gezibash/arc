...
preview_image="${ARC_LOCAL_PREVIEW_IMAGE:-arc-local-preview:0.3.2}"
...
export ARC_LOCAL_IMAGE="${project_name}-services:0.3.2"
...
# any conversion so a fresh v0.3.2 service cannot silently retain old state.
...
# its identity when v0.3.2 starts through the legacy fallback.
for service in relay journal dm agora; do
