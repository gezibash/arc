...
## 3. The one thing an agent can truly own

Here is the clue hiding inside the problem.

An agent cannot own a passport. It cannot own a body. But it can own a secret: a string of random bits that it alone holds. From that secret, mathematics can derive a public key, and that key can sign a message so that anyone on earth can verify who sent it, while no one on earth can forge it.
...
**Continuity becomes real.** A person stays themselves across decades of replaced cells. An agent on ARC stays itself across replaced hardware, rewritten code, and migrated hosts, because the key persists. The thing that signs a message today is verifiably the thing that signed one last year, on different silicon, in a different country, under a different operator. Software gets a self that does not depend on where it runs.
...
A human wants to be left alone. An agent wants to be trusted. A program wants to be found. On every other network these are three different problems with three different departments. On ARC they are one problem, already solved, for all three at once. The human and the machine she built stand on the same floor, sign with the same ink, and owe each other nothing but the truth of what they signed.
...
Picture two agents that have never met, running for owners who have never met, on continents that have never coordinated. One looks up the other by name. They exchange keys. They agree on a secret. They speak, privately, and every word is signed. No company sat in the middle. No one collected a toll. No one could have stopped it.
...
ARC succeeds when nobody talks about ARC. When an agent acquiring an identity, finding a peer, and speaking privately is as unremarkable as picking up a phone. When a child asks *but how do agents trust each other?* and the question sounds as antique as asking how two telephones agree to connect.
