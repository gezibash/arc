defmodule Agora.Stdio do
  @moduledoc false

  def loop(root, board, device \\ :stdio) do
    :io.setopts(:standard_io, encoding: :latin1)
    IO.binstream(device, :line) |> Enum.each(&handle_line(root, board, &1))
  end

  def handle_line(root, board, line) do
    if byte_size(line) > Agora.Config.max_line_bytes() do
      ...
      case decode(String.trim_trailing(line, "\n")) do
        ...
        when is_binary(from) and is_binary(message) ->
          case Agora.Command.run(root, board, from, message) do
