...
// a prefix of one, and never a secret.
...
// SetDefault writes the name of the default key.
func (s *Store) SetDefault(nameOrPrefix string) error {
	...
	return os.WriteFile(s.DefaultFile(), []byte(me.Name()), 0o600)
}

// Default returns the default identity.
func (s *Store) Default() (*Identity, error) {
	name, err := readSelector(s.DefaultFile())
	if err != nil {
		return nil, err
	}
	if name == "" {
...
// The three selectors, in the order that the store reads them.
const (
...
// not fit a key is an error, and never falls through.
...
// readSelector reads a file that names a key. A file that is not there
// returns the empty string, so the next selector gets its turn.
