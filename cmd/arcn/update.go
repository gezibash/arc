...
// arcn reads its own releases from a citizen that serves them, and replaces
...
func updateCommand() *cobra.Command {
	command := &cobra.Command{
		...
		Long: "Without a subcommand, arcn reads the channel and says what it\n" +
			"would do. arcn update apply replaces the program.",
		Args: cobra.NoArgs,
...
func update(command *cobra.Command, apply bool) error {
	...
	if errors.Is(err, release.ErrNoRelease) {
		fmt.Printf("arcn %s is the newest release of %s for %s/%s\n", version, channel, platform.OS, platform.Arch)
		return nil
	}
	if err != nil {
		return err
	...
	fmt.Println()
	fmt.Printf("arcn %s is installed at %s\n", newest.Version, program)
...
func releaseCommand() *cobra.Command {
	...
	sign := &cobra.Command{
		...
		Long: "arcn checks the size and the hash of each archive that the channel\n" +
