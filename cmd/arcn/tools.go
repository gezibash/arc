	...
	command.AddCommand(
		&cobra.Command{
			...
			RunE: func(command *cobra.Command, _ []string) error {
				...
				if len(list) == 0 {
					fmt.Println("no capabilities: install one with arcn install <provider>")
					return nil
				}
				for _, e := range list {
...
func installed(installs catalog.Installs, name string) (catalog.Install, error) {
	...
	return catalog.Install{}, fmt.Errorf("nothing installed as %q: see arcn tool list", name)
}
...
func resolveCommand() *cobra.Command {
	return &cobra.Command{
		...
		Long: "arcn looks in the identities of this machine, in its installs, and in\n" +
...
func appsCommand() *cobra.Command {
	...
	command.AddCommand(&cobra.Command{
		...
		Long: "The bundle holds an Arcfile, a manifest, and a runtime that\n" +
			"answers one message. Serve it with arcn serve <directory>.",
