// Command arc-legacy is the command line tool of the older ARC stack. It is
// deprecated: arc, built from cmd/arc, replaces it, and v0.11.0 removes it.
//
//	arc-legacy keys gen            make an identity
//	arc-legacy join <host:port>    remember a relay and its public key
//	arc-legacy serve <uri>         serve a provider over the relay
//	arc-legacy call <address>      call a capability of another citizen
//	arc-legacy discover <query>    search the directory of the relay
//	arc-legacy status              ask the relay about itself
package main
...
func main() {
	// This program is the older stack. docs/delivery/SPEC.md, section 15.1,
	// removes it in v0.11.0.
	fmt.Fprintln(os.Stderr, "arc-legacy is deprecated, and v0.11.0 removes it: use arc, see docs/delivery/SPEC.md section 15.1")
	command := root()
...
func root() *cobra.Command {
	...
	command.PersistentFlags().String("key", "", "the identity to use, by petname (ARC_LEGACY_KEY)")
