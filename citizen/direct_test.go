...
func policyFile(t *testing.T, peer []byte, listens bool) string {
	t.Helper()
	return leasedPolicyFile(t, peer, listens, 30000)
}

// leasedPolicyFile writes the same rule, with a lease of its own.
func leasedPolicyFile(t *testing.T, peer []byte, listens bool, leaseMS int) string {
	t.Helper()
...
// When the relay goes away, a direct route that stands keeps its
// conversation until its lease ends. Then the citizen stops, and says that the
// relay connection ended.
func TestDirectRoutesServeAfterTheRelayEnds(t *testing.T) {
	relayIdentity, _ := identity.Generate()
	server, err := relay.Listen(context.Background(), relay.Options{
		Identity: relayIdentity, Address: "127.0.0.1:0", Log: quiet,
	})
	...
	serving, err := citizen.Serve(context.Background(), citizen.Options{
		...
		RelayPublicKey: server.PublicKey(),
		Serve:          "exec://" + binary + "?manifest=" + manifest,
		DirectPolicy:   leasedPolicyFile(t, caller.PublicKey, true, lease),
		Log:            quiet,
	})
	if err != nil {
		t.Fatalf("serve: %v", err)
	...
	peers := connection.Peers()
	rules, err := direct.LoadPolicy(leasedPolicyFile(t, serving.PublicKey(), false, lease))
	if err != nil {
		t.Fatal(err)
	}
	peers.Direct(rules, quiet)
	...
	answer, err := peers.Call(ctx, address, []byte("the relay is gone"), "")
	if err != nil {
		t.Fatalf("the direct route ended with the relay: %v", err)
	}
	if !strings.HasSuffix(string(answer.Body), "the relay is gone") {
		t.Errorf("body = %q", answer.Body)
	}
	if err := serving.Err(); err != nil {
		t.Fatalf("the citizen stopped while its route stood: %v", err)
	}

	select {
	case <-serving.Done():
	case <-time.After(lease*time.Millisecond + 3*time.Second):
		t.Fatal("the citizen served past the end of its last lease")
	}
	if held := time.Since(promoted); held < lease*time.Millisecond-500*time.Millisecond {
		t.Errorf("the citizen stopped %s after the promotion, before the lease ended", held)
	}
